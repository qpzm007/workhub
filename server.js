...
사용 가능한 작업(type) 종류:
1. "CREATE_DIR": 새 폴더 생성. (필요 속성: "target" - 생성할 폴더명)
2. "CREATE_FILE": 새 파일 생성 및 내용 작성. (필요 속성: "target" - 생성할 파일명(경로 포함 가능), "content" - 파일에 들어갈 초기 내용 문자열)
3. "MOVE": 기존 파일이나 폴더를 다른 위치로 이동. (필요 속성: "source" - 이동할 기존 항목명, "target" - 대상 폴더명 또는 새 경로)

JSON 형식 예시:
[
  { "type": "CREATE_DIR", "target": "구매팀" },
  { "type": "CREATE_DIR", "target": "구매팀/발주서" },
  { "type": "CREATE_FILE", "target": "구매팀/발주서/2026년도_가이드.txt", "content": "구매 발주 가이드라인 문서입니다." },
  { "type": "MOVE", "source": "기존영수증.png", "target": "구매팀" }
]

(주의: 반드시 마크다운이나 백틱 없이 순수 JSON 배열 문자열만 반환하세요. 생성하는 파일의 content는 문자열이어야 합니다.)
`;
